...
## 6-1. 배열 메서드: `filter`와 `map`을 실제 코드에서 구분하기

프로젝트 목록은 배열입니다. 이 배열에 여러 메서드를 연달아 사용하지만, 역할은 서로 다릅니다.

- `filter()`: **조건에 맞는 항목만 남겨 새 배열을 만듭니다.** `renderProjects()`에서 이름이 있는 저장소만 `visibleRepositories`에 남깁니다. 원본 배열은 그대로입니다.
- `sort()`: 화면에서 보여 줄 **순서**를 바꿉니다. B1-1/E1 저장소를 앞쪽에 배치합니다.
- `map()`: 남아 있는 각 저장소를 **프로젝트 카드 HTML 문자열로 변환**합니다. 저장소 1개가 카드 1개가 되는 셈입니다.
- `forEach()`: 배열의 각 항목에 같은 작업을 수행합니다. 메뉴 링크마다 클릭 이벤트를 붙이거나, 폼 입력칸마다 검증 이벤트를 붙일 때 사용합니다.
- `find()`: 조건에 맞는 **첫 번째 항목 하나**를 찾습니다. 폼 제출 실패 시 첫 번째 오류 입력칸을 찾아 포커스를 이동할 때 사용합니다.

가장 중요한 차이는 이것입니다.

```text
filter : 여러 개 중 필요한 것만 골라 새 배열
map    : 배열의 각 항목을 다른 형태로 바꿔 새 배열
forEach: 각 항목마다 작업 실행, 새 배열이 목적은 아님
find   : 조건에 맞는 첫 번째 항목 하나 찾기
```

프로젝트 영역의 흐름은 **GitHub API 배열 → filter → sort → map → join → innerHTML**입니다. 평가에서 `map`이나 `filter`를 물으면 이 순서와 각 단계의 역할을 설명하면 됩니다.

**확인할 것:** `renderProjects()`에서 `visibleRepositories`, `ordered`, `ordered.map(...)`이 각각 무엇을 담는지 한 문장씩 말해 보세요.
...
7. `filter()`와 `map()`은 각각 배열에 무엇을 하나요? 이 프로젝트에서는 어디에 쓰였나요?
